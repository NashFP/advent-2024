...
let make_grid lines =
  ...
  Mwlib.init_matrix width height ~f:get_item

let get_groups grid =
  let (width,height) = Mwlib.matrix_dim grid in
  let is_valid (x,y) = (x >= 0) && (x < width) && (y >= 0) && (y < height) in
  let is_unvisited visited pos = not (Set.mem visited pos) in
  let rec try_visit ch (visited, group) (x,y) =
    ...
    if is_valid (x,y) && is_unvisited visited (x,y) &&
         Char.(grid.(x).(y) = ch) then
      let v0 = Set.add visited (x,y) in
      let g0 = (x,y) :: group in
      List.fold ~f:(try_visit ch) ~init:(v0, g0) [(x-1,y);(x+1,y);(x,y-1);(x,y+1)]
    else
  ...
  snd (List.fold ~f:make_group ~init:(PairsSet.empty, []) (Mwlib.matrix_to_list grid))
...
let get_group_value grid group =
  ...
  let get_perimeter_value (x,y) = 
    ...
    let perimeter_edge (x,y) =
      ...
      if not (is_valid (x,y)) || Char.(ch <> grid.(x).(y)) then 1 else 0
    in
    (* Sum the edge values for all four sides of the square *)
    List.reduce_exn ~f:(+) (List.map ~f:perimeter_edge [(x-1,y);(x+1,y);(x,y-1);x,y+1])
  in
  let get_group_perimeter group =
    List.reduce_exn ~f:(+) (List.map ~f:get_perimeter_value group) in
...
let get_group_value_b grid group =
  ...
  let get_corner_value (x,y) = 
    ...
    let outer_corner_value ((x1,y1),(x2,y2)) =
      if (not (is_valid (x1,y1)) || Char.(ch <> grid.(x1).(y1))) &&
           (not (is_valid (x2,y2)) || Char.(ch <> grid.(x2).(y2))) then 1 else 0
    in
    ...
    let inner_corner_value ((x1,y1),(x2,y2),(x3,y3)) =
      if (is_valid (x1,y1) && is_valid (x2,y2) &&
            Char.(ch = grid.(x1).(y1)) && Char.(ch = grid.(x2).(y2))) &&
           (not (is_valid (x3,y3)) || Char.(ch <> grid.(x3).(y3))) then 1 else 0
    in
    (List.reduce_exn ~f:(+) (List.map ~f:outer_corner_value [((x-1,y),(x,y-1));
                                                        ((x,y-1),(x+1,y));
                                                        ((x+1,y),(x,y+1));
                                                        ((x,y+1),(x-1,y))])) +
      (List.reduce_exn ~f:(+) (List.map ~f:inner_corner_value
                               [((x-1,y),(x,y-1),(x-1,y-1));
                                ((x,y-1),(x+1,y),(x+1,y-1));
                                ((x+1,y),(x,y+1),(x+1,y+1));
                                ((x,y+1),(x-1,y),(x-1,y+1))])) in
  let get_group_corners group =
    List.reduce_exn ~f:(+) (List.map ~f:get_corner_value group)
  in
...
let day12 () =
  ...
  let resulta = List.reduce_exn ~f:(+)
      (List.map ~f:(get_group_value grid) groups) in
  let resultb = List.reduce_exn ~f:(+)
      (List.map ~f:(get_group_value_b grid) groups) in
