...
let line_regex = Re.Str.regexp ".*: *[A-Z][+=]\\([0-9]*\\), [A-Z][+=]\\([0-9]*\\)"
...
let parse_group group_lines =
  (parse_numbers (List.hd_exn group_lines),
   parse_numbers (List.nth_exn group_lines 1),
   parse_numbers (List.nth_exn group_lines 2))

let round x = Float.to_int (x +. 0.5)

let solution ((x1,y1),(x2,y2),(x3,y3)) =
  (* Find the determinant of the matrix.
     If it is 0, there is no solution. *)
  let det = Float.of_int (x1*y2 - x2*y1) in
  if Float.(det = 0.0) then
    None
  else
    (* Compute the inverse of the matrix *)
    let x1i = (Float.of_int y2) /. det in
    let x2i = (0.0 -. Float.of_int y1) /. det in
    let y1i = (0.0 -. Float.of_int x2) /. det in
    let y2i = (Float.of_int x1) /. det in
    (* Multiply the matrix by the desired result to find a and b *)
    let a = round (x1i *. (Float.of_int x3) +. y1i *. (Float.of_int y3)) in
    let b = round (x2i *. (Float.of_int x3) +. y2i *. (Float.of_int y3)) in
...
let day13 () =
  ...
  let group_numbers = List.map ~f:parse_group groups in
  let solutions = List.map ~f:solution group_numbers in
  let resulta = List.reduce_exn ~f:(+) (List.map ~f:cost solutions) in
  let group_numbers_b = List.map ~f:make_b_group group_numbers in
  let solutions_b = List.map ~f:solution group_numbers_b in
  let resultb = List.reduce_exn ~f:(+) (List.map ~f:cost solutions_b) in
