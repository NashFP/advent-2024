...
let rec work_backward width height grid working_set best_paths =
  let is_valid (x,y) = x >= 0 && x < width && y >= 0 && y < height &&
                         grid.(y).[x] != '#' in
  let processed (x,y) = PairsSet.mem (x,y) best_paths in
  let add_to_working_set curr_working_set (x,y) =
    if is_valid (x,y) && not (processed (x,y)) then
      PairsSet.add (x,y) curr_working_set
    else
      curr_working_set in
  
    
  let try_coord (new_working_set, curr_best_paths) (x,y) =
    
  List.fold_left try_coord (PairsSet.empty, best_paths)
    (PairsSet.to_list working_set)
